...
.NOTES
2019-05-22  - Added PnPUnifiedGroup cmd to provision SPO sites for data migration targets.
...
If ($ProvisionSPOGroupSites)
{
	If ($ProvisionSPOGroupSites)
	{
		If (!($Tenant))
		{
			while ($tenant -notlike "*.onmicrosoft.com") { $tenant = read-host "Enter tenant name [tenant.onmicrosoft.com]" }
		}

		If (!($Credential)) { $Credential = Get-Credential -Message "Office 365 Global Administrator" }
		...
		"Connect-PnpOnline -Url $($Site) -credentials `$Credential -Scopes Group.ReadWrite.All"
		Invoke-Expression $cmd
		Write-Host "Running Get-PnPUnifiedGroup to provision new sites for migrated groups."
		try
		{
			$PnpGroups = Get-PnpUnifiedGroup
			[int]$PnpGroupsCount = $PnpGroups.Count
		}
		catch { "Unspecified error. Run Get-PnpUnifiedGroup manually to check for provisioned groups." }
		finally { If ($PnpGroups -ge 1) { Write-Host "Processed $($PnpGroupsCount) groups." } }
	}
}
